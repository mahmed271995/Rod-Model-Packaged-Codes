...
1. **Forward Code:** Contains packaged codes that solve the rod model equations in the forward approach, i.e., given a known constitutive law and boundary conditions, solve for the unknown parameters.  
   1. **Linear:** Considers a linear constitutive relationship  
      $$\vec{q} = [\mathbf{B}] \vec{\kappa}$$  
   2. **Nonlinear:** Considers a nonlinear constitutive relationship  
      $$\vec{q} = \sum_{i=0}^{5} \mathbf{B_i} \vec{\kappa}^i$$  
      more specifically,  
      $$
      \left\lbrace 
      \begin{array}{c}
      ...
      \begin{array}{c}
      E I_1 \kappa_1 \\[4pt]
      E I_2 \kappa_2 + a \kappa_2^3 + b \kappa_2^5 \\[4pt]
      G J \kappa_3
      \end{array}
      \right\rbrace
      $$  
   3. **Nonlinear and Non-Homogeneous:** Considers nonlinear and non-homogeneous constitutive relationship  
      $$\vec{q} = \sum_{i=0}^{5} \mathbf{B(s)_i} \vec{\kappa}^i$$  
      more specifically,  
      $$
      \left\lbrace 
      \begin{array}{c}
      ...
      \begin{array}{c}
      E I_1 \kappa_1 \\[4pt]
      (a s^3 - b s^2 + c s + d)(e \kappa_2^3 + f \kappa_2) \\[4pt]
      G J \kappa_3
      \end{array}
      \right\rbrace
      $$  

2. **Inverse Code:** Contains the code that solves the rod model equations in the inverse approach, i.e., given deformation data and boundary conditions, solve for the constitutive law parameters.
